#!/usr/bin/env python3
"""Verify the deepened English rock band batch for the music wiki."""
...
def main() -> int:
    ...
    if not log.exists() or "英语摇滚乐队逐曲分析批次" not in log.read_text(encoding="utf-8"):
        errors.append("wiki/log.md missing English rock band batch entry")

    if errors:
        print("English rock band batch verification FAILED:")
        for error in errors:
            print(f"- {error}")
        return 1

    print("English rock band batch verification passed.")
    for name in ENROCK_BATCH:
