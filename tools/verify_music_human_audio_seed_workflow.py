#!/usr/bin/env python3
"""Verify the AI music human audio seed workflow notes."""
...
PAGES = {
    "workflow": WIKI / "分析" / "音乐" / "AI 音乐人类音频草稿到成品工作流.md",
    "source": WIKI / "来源" / "外部资料" / "音乐" / "AI 音乐人类音频草稿与输入锚点资料组.md",
...
WORKFLOW_TERMS = [
    "human-seed-intent.md",
    "audio-rights-ledger.md",
    "seed-type-classification.md",
    "capture-and-cleanup.md",
    "retention-target.md",
    "tool-routing.md",
    "prompt-pairing.md",
    "generate-takes.md",
    "anchor-retention-review.md",
    "repair-or-reseed.md",
    "catalog-seed-hygiene.md",
    "release-rights-link.md",
    "Audio Upload",
    "Create Music with Audio",
    "Add Vocals",
    "Cover",
    "Extend",
    "Reuse Prompt",
...
def main() -> int:
    ...
    workflow_link = "[[AI 音乐人类音频草稿到成品工作流]]"
    source_link = "[[AI 音乐人类音频草稿与输入锚点资料组]]"
    ...
    for key in workflow_link_pages:
        text = read(PAGES[key])
        if workflow_link not in text and "AI 音乐人类音频草稿到成品工作流" not in text:
    ...
    log = read(PAGES["log"])
    if "AI 音乐人类音频草稿到成品工作流" not in log:
        errors.append("wiki/log.md missing human audio seed workflow entry")

    if errors:
        print("AI music human audio seed workflow verification failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("AI music human audio seed workflow verification passed.")
