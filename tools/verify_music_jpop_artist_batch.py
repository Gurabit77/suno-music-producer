#!/usr/bin/env python3
"""Verify the deepened Japanese pop artist batch for the music wiki."""
...
def main() -> int:
    ...
    for name in JPOP_BATCH:
        ...
        if title_hits < 3:
            errors.append(f"{name} has fewer than 3 named works: {title_hits}")
    ...
    if not log.exists() or "日语流行艺人逐曲分析批次" not in log.read_text(encoding="utf-8"):
        errors.append("wiki/log.md missing J-pop artist batch entry")

    if errors:
        print("J-pop artist batch verification FAILED:")
        for error in errors:
            print(f"- {error}")
        return 1

    print("J-pop artist batch verification passed.")
    for name in JPOP_BATCH:
