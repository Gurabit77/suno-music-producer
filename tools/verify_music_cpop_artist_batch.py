#!/usr/bin/env python3
"""Verify the deepened Mandarin/Cantonese pop artist batch for the music wiki."""
...
def main() -> int:
    ...
    if not log.exists() or "华语流行艺人逐曲分析批次" not in log.read_text(encoding="utf-8"):
        errors.append("wiki/log.md missing C-pop artist batch entry")

    if errors:
        print("C-pop artist batch verification FAILED:")
        for error in errors:
            print(f"- {error}")
        return 1

    print("C-pop artist batch verification passed.")
    for name in CPOP_BATCH:
