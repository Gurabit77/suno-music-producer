#!/usr/bin/env python3
"""Verify the AI music lyric narrative and anti-cliche workflow notes."""
...
PAGES = {
    "workflow": WIKI / "分析" / "音乐" / "AI 音乐歌词叙事与反陈词滥调工作流.md",
    "source": WIKI / "来源" / "外部资料" / "音乐" / "AI 音乐歌词叙事与反陈词滥调资料组.md",
    ...
    "prosody": WIKI / "分析" / "音乐" / "四语种歌词 Prosody 与 AI 音乐咬字审查表.md",
...
def main() -> int:
    ...
    workflow_link = "[[AI 音乐歌词叙事与反陈词滥调工作流]]"
    source_link = "[[AI 音乐歌词叙事与反陈词滥调资料组]]"
    ...
    if workflow_link not in root_index and "AI 音乐歌词叙事与反陈词滥调工作流" not in root_index:
    ...
    log = read(PAGES["log"])
    if "AI 音乐歌词叙事与反陈词滥调工作流" not in log:
        errors.append("wiki/log.md missing lyric narrative workflow entry")

    if errors:
        print("AI music lyric narrative workflow verification failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("AI music lyric narrative workflow verification passed.")
