#!/usr/bin/env python3
"""Verify the AI music arrangement/mixing/post-production workflow notes."""
...
PAGES = {
    "workflow": WIKI / "分析" / "音乐" / "AI 音乐编曲密度与混音后期修作工作流.md",
    "source": WIKI / "来源" / "外部资料" / "音乐" / "AI 音乐编曲混音与后期修作资料组.md",
...
def main() -> int:
    ...
    workflow_link = "[[AI 音乐编曲密度与混音后期修作工作流]]"
    source_link = "[[AI 音乐编曲混音与后期修作资料组]]"
    ...
    if workflow_link not in root_index and "AI 音乐编曲密度与混音后期修作工作流" not in root_index:
    ...
    log = read(PAGES["log"])
    if "AI 音乐编曲密度、混音与后期修作工作流" not in log:
        errors.append("wiki/log.md missing post-production workflow entry")

    if errors:
        print("AI music post-production workflow verification failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("AI music post-production workflow verification passed.")
